...
- **Chats persist.** `compose.chat(agent)` keeps a conversation going across sends — and across processes (`load_chat(id)`) — with inline tool approval (`approver=`, denials can carry a message via `ApprovalReply`), per-chat `system=`/`model=` overrides, and a `context_manager=` hook for compaction. Each send is an ordinary traced, budgeted run.
...
- **Tool calls can be intercepted.** `tool_interceptor=` (0.11.0) is a `before`/`after` hook around every tool call — proceed, proceed with modified arguments, or deny before the approver even sees it; `after` can rewrite a tool's result. `None` (default) is a no-op; not journaled, so it never re-fires on resume for tools that already ran. See [agents](docs/agents.md#per-call-overrides).
...
| [docs/chats.md](docs/chats.md) | Persistent chat sessions: `compose.chat`, `load_chat`, inline approval, context management, per-call overrides |
